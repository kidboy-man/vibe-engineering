...
argument-hint: [feature-name | path-to-requirement-file]
---
...
Skim `README.md`, `CLAUDE.md`, and any `docs/` folder for product context, existing users, and
terminology, so you only ask what the files cannot answer.

## Step 3 — Interview
Ask only the questions whose answers you cannot infer, all at once (use `AskUserQuestion` with a
multi-question form, or one numbered list). Cover:
