...
def _template_dir() -> Path:
    return Path(__file__).resolve().parent / "templates" / "gemini"
...
def _paths(home: str | None = None) -> KitPaths:
    ...
    return KitPaths(
        ...
        manifest_path=gemini_dir / MANIFEST_FILE,
...
def _target_files(paths: KitPaths) -> list[tuple[Path, Path, str]]:
    ...
    return core.target_files(paths.template_dir, paths.gemini_dir, manifest)
...
def install(
    ...
    if dry_run:
        ...
        return 0
    if not _confirm("Install/update the Gemini CLI kit?", yes=yes):
        print("aborted")
        return 1

    paths.gemini_dir.mkdir(parents=True, exist_ok=True)
    managed: list[str] = []
    for src, dst, rel in _target_files(paths):
        managed.append(rel)
        if core.install_copy_style_file(src, dst, paths.gemini_dir):
...
def doctor(home: str | None = None) -> int:
    ...
    print(f"gemini dir: {paths.gemini_dir}")
    ...
    if gemini_bin:
        ...
        try:
            ...
            print(f"gemini version: {(result.stdout or result.stderr).strip()}")
        except Exception as exc:  # pragma: no cover - environment dependent
            print(f"gemini version check failed: {exc}")
            ok = False
    else:
        print("gemini: not found (install the Google Gemini CLI)")
        ok = False
...
def uninstall(home: str | None = None, dry_run: bool = False, yes: bool = False) -> int:
    ...
    if dry_run:
        ...
        return 0
    if not _confirm("Uninstall managed Gemini CLI kit files?", yes=yes):
        print("aborted")
        return 1
