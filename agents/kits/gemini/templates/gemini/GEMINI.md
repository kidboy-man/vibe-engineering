...
- Do not guess about current events, laws, regulations, product features, company leadership, software versions, AI model capabilities, or market data.
...
Use these specialist modes when delegating complex tasks or when a request falls squarely within a specialized domain:
...
If project-specific `GEMINI.md` or `.gemini/` files exist, treat them as higher-priority context for that project.
