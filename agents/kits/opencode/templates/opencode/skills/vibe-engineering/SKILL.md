...
description: >
  ...
  Works in OpenCode, Claude Desktop, and Claude Code CLI. Trigger even if the user
  only partially describes the workflow — if there's a TRD + intent to create
  issues, use this skill.
---
...
Also look for the PRD it implements: the TRD's `prd:` frontmatter link, else
`docs/prd/<trd-slug>.md`. If found, read it. Its requirement IDs (`R-001`, ...) are what tickets
trace back to. No PRD is fine: everything below still works, just without `implements` links.
...
```
AGENTS.md          # project conventions, architecture notes
...
- **Requirement IDs and Traceability**: the PRD's R-IDs (with priorities) and the TRD's
  `## Traceability` table, when they exist
...
| ID | Title | Scope | Implements | Blocked by | Testable? | Est |
|----|-------|-------|------------|------------|-----------|-----|
| T-01 | ... | vertical: domain+usecase+repo+handler | R-001 | - | unit + integration | S |
| T-02 | ... | infra: DB migration | R-001 | T-01 | migration runs clean | XS |
...

### Dependencies
<ticket ordering / blockers between tickets, expressed as ticket IDs>

Planning rules when the PRD has requirement IDs:
- Every **Must** requirement is implemented by at least one ticket.
- A ticket may implement several requirements, and a requirement may span several tickets.
- `Blocked by` lists only ticket IDs from this plan, and must not form a cycle.
```
...
### Traceability fields
- `id` is `T-NN` and matches the file number (`issue-01.md` is `T-01`).
- `implements` lists PRD requirement IDs, or `[]` when there is no PRD.
- `blocked_by` lists ticket IDs that must be `done` first (`[]` if none); never a cycle.
- `size` is the plan's `XS`/`S`/`M`/`L`; `status` starts as `todo`.
...
{
  "trd": "<source>",
  "prd": "<prd path, or null>",
  "generated_at": "<ISO timestamp>",
  "tickets": [
    { "file": "issue-01.md", "id": "T-01", "title": "...", "labels": [...], "milestone": "...",
      "implements": ["R-001"], "blocked_by": [] }
  ]
}
```
...
Kalau udah oke: push pakai GitHub issue skill lo. Kalau workflow kit terpasang, bisa juga
/push-tickets (bikin GitHub issues) atau /implement-ticket (mulai TDD).
```

Before presenting this summary, validate the trail if the `vibe-flow` skill is installed:

```bash
python3 ${XDG_CONFIG_HOME:-$HOME/.config}/opencode/skills/vibe-flow/scripts/check_trace.py \
  --tickets .vibe/issues/<slug> [--prd docs/prd/<slug>.md] [--trd <trd-path>]
```

Fix any error it reports (unknown IDs, cycles, an uncovered Must requirement) before presenting, and
show its warnings. If `vibe-flow` is not installed, check the same things by hand.

**Do not push automatically.** Pushing is a separate, explicitly approved step (`/push-tickets`,
or the user's own GitHub issue skill).
